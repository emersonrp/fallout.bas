...
1630   RSIDEPOS%(I) = (RCELLLENGTH% * (I-1)) + INT(RCELLLENGTH% / 2) - INT(LEN(WORDS$(I)) / 2) + (RND(7) - 4)
...
40005 LPRINT "CURY "; STR$(CURY!); " CURX "; STR$(CURX!); " CURSIDE "; STR$(CURSIDE!): ' TODO TODO TODO REMOVE THIS
40010 IF CURSIDE! = 0 THEN CURPALOC% = (CURY!+6)*80 + 6 + CURX! ELSE CURPALOC% = (CURY!+6)*80 + 26 + CURX!
40020 IF CURSIDE! = 0 THEN CURCHAR$ = MID$(LSIDE$, (CURY!-1)*12 + CURX!, 1) ELSE CURCHAR$ = MID$(RSIDE$, (CURY!-1)*12 + CURX!, 1)
40030 IF VAL(CURCHAR$) < 65 THEN PRINT @ CURPALOC%, CHR$(16);CURCHAR$;CHR$(17);: GUESS$ = CURCHAR$: GOSUB 44000: CURRWORD% = 0: GOSUB 49040: RETURN
40040 FOR I = 1 TO WORDLISTLENGTH%
40050   FOR J = 1 TO WORDLENGTH%
40060     IF PAPOSITIONS(I, J) = CURPALOC% THEN CURRWORD% = I: GOSUB 43000: J = WORDLENGTH%: I = WORDLISTLENGTH%
40070   NEXT J
40080 NEXT I
40090 RETURN
...
41080 NEXT J
41090 RETURN
41100 REM -- subroutine to initialize the list of hex addresses
41110 RESTORE 41130
41111 REM TODO Get a better list of these
41112 REM Don't make any of these between 0 and 4096 or HEX$ will format the screen badly
41120 DIM HEXADDRS!(7)
41130 DATA 48304,49968,64198,20992,63056,20464,62720
41140 FOR J = 1 TO 7
41150   READ HEXADDRS!(J)
41155   IF HEXADDRS!(J) > 32767 THEN HEXADDRS!(J) = HEXADDRS!(J) - 65536
41160 NEXT J
41170 RETURN
